...
function RecordPayment({tenant,legacy,onClose,onSave}) {
 const [date,setDate]=useState(localDate);const [method,setMethod]=useState('Cash');const [phone,setPhone]=useState(tenant.phone||'');const [email,setEmail]=useState(tenant.email||'');
 return <Modal title={t(legacy?'Receipt details':'Record payment')} subtitle={`${tenant.name} · ${money(tenant.rent)}`} onClose={onClose}><form className="modal-form" onSubmit={e=>{e.preventDefault();onSave({date,method,phone,email})}}>{legacy&&<p>{t('Enter the original payment date and method to create this receipt.')}</p>}<div className="form-row"><label>{t('Mobile number')}<input type="tel" autoComplete="tel" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+973 0000 0000"/></label><label>{t('Email address')}<input type="email" autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="name@example.com"/></label></div><label>{t('Payment date')}<input required type="date" max={localDate()} value={date} onChange={e=>setDate(e.target.value)}/></label><label>{t('Payment method')}<select value={method} onChange={e=>setMethod(e.target.value)}>{['Cash','Debit Card','Credit Card','BenefitPay','ApplePay'].map(value=><option key={value} value={value}>{t(value)}</option>)}</select></label><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>{t('Cancel')}</button><button className="primary" type="submit"><Check size={16}/>{t('Save payment')}</button></div></form></Modal>;
}
...
function App() {
 ...
 const togglePaid=(id,paymentMonth=month)=>{
  if(data.payments[paymentMonth]?.[id]){setData(d=>({...d,payments:{...d.payments,[paymentMonth]:{...(d.payments[paymentMonth]||{}),[id]:false}}}));return}
  setModal({type:'payment',tenant:data.tenants.find(tenant=>tenant.id===id),month:paymentMonth});
 };
 const savePayment=({date,method,phone,email})=>{
  const receipt=createReceipt({tenant:{...modal.tenant,phone:phone.trim(),email:email.trim()},property:data.properties.find(p=>p.id===modal.tenant.propertyId),month:modal.month,date,method,issuer:{username:session.user.user_metadata?.username||'Rent Realm',email:session.user.email}});
  setData(d=>({...d,tenants:d.tenants.map(tenant=>tenant.id===modal.tenant.id?{...tenant,phone:phone.trim(),email:email.trim()}:tenant),payments:{...d.payments,[modal.month]:{...(d.payments[modal.month]||{}),[modal.tenant.id]:receipt}}}));setModal({type:'receipt',receipt});
 };
 const onReceipt=(tenant,paymentMonth=month)=>{const receipt=data.payments[paymentMonth]?.[tenant.id];if(receipt?.number)setModal({type:'receipt',receipt:{...receipt,propertyAddress:receipt.propertyAddress??data.properties.find(property=>property.id===tenant.propertyId)?.address??''}});else setModal({type:'payment',tenant,month:paymentMonth,legacy:true})};
