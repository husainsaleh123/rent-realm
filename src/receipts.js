...
export function createReceipt({ tenant, property, month, date, method, issuer }) {
...
export function receiptDocument(receipt) {
  ...
  autoTable(doc,{startY:start+8,theme:'striped',body:[[`Payment Date: ${date}`,''],[`Property: ${receipt.property} · ${receipt.tenant.unit}`,''],[`Payment Amount: ${amount}`,`Payment Method: ${receipt.method}`],[`Memo: ${receipt.invoice}`,'']],styles:{fontSize:10,textColor:ink,cellPadding:3},alternateRowStyles:{fillColor:[243,244,246]},columnStyles:{0:{cellWidth:105}},margin:{left:14,right:14}});
  autoTable(doc,{startY:doc.lastAutoTable.finalY+9,theme:'striped',head:[['Invoice Date','Invoice Number','Reference','Amount']],body:[[ `01/${receipt.month.slice(5)}/${receipt.month.slice(0,4)}`,receipt.invoice,reference,amount],[date,receipt.number,receipt.invoice,`-${amount}`],['',`Balance Amount for ${receipt.invoice}`,'','0.000 BD']],styles:{fontSize:9,textColor:ink,cellPadding:3},headStyles:{fillColor:[255,255,255],textColor:ink,fontStyle:'normal'},alternateRowStyles:{fillColor:[243,244,246]},columnStyles:{3:{halign:'right'}},didParseCell:cell=>{if(cell.section==='body'&&cell.row.index===2)cell.cell.styles.fontStyle='bold'},margin:{left:14,right:14}});
